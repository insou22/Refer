 ...
 */
public class RequestConfirmationPage extends GUIPage {

    private final String title = ChatColor.GREEN + "Refer > Request Confirm";
    private final ReferPlayer player;
    private final ConfirmationType type;

    public RequestConfirmationPage(ReferPlayer player, ConfirmationType type) {
        super(player);
        this.player = player;
        this.type = type;
        setup();
    ...
    private void setup() {
        ...
        switch (type) {
            ...
            case CANNOT_REFER_PLAYER:
                ...
                result.withMeta(
                        new MetaBuilder(result)
                                .withDisplayName(ChatColor.RED + "You can't request that player!")
                );
                break;
            case SUCCESS:
                ...
                result.withMeta(
                        new MetaBuilder(result)
                                ...
                                .withLore(ChatColor.AQUA + "Request has been sent!")
                );
                break;
            default:
                result = new ItemBuilder(Material.WOOL).withDurability(14);
    ...
    public GUIPageType getType() {
        return GUIPageType.REQUEST_CONFIRM;
    }
